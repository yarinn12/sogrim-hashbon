...
test("a fresh iPhone or iPad home-screen install receives the current app shell", async ({
  ...
  const installUrl = installBaseUrl
    ? new URL("/?pwa_release=358", installBaseUrl).toString()
    : "/?pwa_release=358";
  ...
  const installedState = await page.evaluate(async () => {
    ...
    const currentCache = await caches.open("settle-friends-live-v358");
  ...
  expect(installedState.scriptUrl).toContain("/sw.js?pwa_release=358");
  expect(installedState.cacheNames).toContain("settle-friends-live-v358");
  expect(installedState.shell).toContain("manifest.webmanifest?pwa_release=358");
  expect(installedState.shell).toContain("styles.css?pwa_release=358");
  expect(installedState.shell).toContain("src/pwaBootstrap.mjs?pwa_release=358");
  expect(installedState.shell).toContain("src/app.mjs?pwa_release=358");
  expect(installedState.shell).toContain("src/publicAccountAuthLayer.mjs?pwa_release=358");
  expect(installedState.shell).toContain("src/publicProfileContextLayer.mjs?pwa_release=358");
  expect(installedState.manifest).toContain('"start_url": "./?pwa_release=358"');
  expect(installedState.manifest).toContain('"display_override": ["standalone"]');
  expect(installedState.bootstrap).toContain('const PWA_RELEASE = "358"');
});
