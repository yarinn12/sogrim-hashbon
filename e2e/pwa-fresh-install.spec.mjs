...
test("a fresh iPhone or iPad home-screen install receives the current app shell", async ({
  ...
  const installUrl = installBaseUrl
    ? new URL("/?pwa_release=347", installBaseUrl).toString()
    : "/?pwa_release=347";
  ...
  const installedState = await page.evaluate(async () => {
    ...
    const currentCache = await caches.open("settle-friends-live-v347");
  ...
  expect(installedState.scriptUrl).toContain("/sw.js?pwa_release=347");
  expect(installedState.cacheNames).toContain("settle-friends-live-v347");
  expect(installedState.shell).toContain("manifest.webmanifest?pwa_release=347");
  expect(installedState.shell).toContain("src/pwaBootstrap.mjs");
  expect(installedState.manifest).toContain('"start_url": "./?pwa_release=347"');
  expect(installedState.manifest).toContain('"display_override": ["standalone"]');
  expect(installedState.bootstrap).toContain('const PWA_RELEASE = "347"');
});
