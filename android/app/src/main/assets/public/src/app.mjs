...
async function handleClick(event) {
  ...
  if (action === "open-event-share") {
    try {
      await prepareEventShare(target.dataset.eventId);
      openEventDialog(target.dataset.eventId, "share", target);
    } catch {
      notice = "לא הצלחנו להכין את קישור ההצטרפות כרגע. כדאי לבדוק את החיבור ולנסות שוב.";
      render();
    }
  }
...
async function prepareEventShare(eventId) {
  const event = getEvent(eventId);
  if (!event) throw new Error("Event not found");
  if (runtimeConfig.storage?.mode === "supabase") {
    ensureEventShareCredentials(event);
    state = await saveSharedEventState(runtimeConfig, state, eventId);
  }
  await saveSharedState(state);
...
async function shareInviteOnWhatsApp(eventId) {
  ...
  try {
    ...
    notice = `אפשר לשלוח ידנית: ${message}`;
  }
  render();
}
