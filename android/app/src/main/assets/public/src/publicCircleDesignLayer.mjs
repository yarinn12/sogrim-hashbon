...
const CSS = `
  ...
  html.circle-design-v1 .recent-event-shortcut::after {
    ...
    background: url("./brand-mark-v3.png") center / cover no-repeat !important;
...
function keepBrandAssetsCurrent() {
  document.querySelectorAll(".product-brand-image").forEach((image) => {
    if (image.getAttribute("src") !== "./brand-mark-v3.png") {
      image.setAttribute("src", "./brand-mark-v3.png");
    }
  });
...
function keepHomePriorityCurrent() {
  ...
  const subtitle = screen.querySelector(":scope > .product-app-identity .product-brand-copy small");
  if (subtitle && !subtitle.dataset.circleDefaultText) {
    subtitle.dataset.circleDefaultText = subtitle.textContent?.trim() || "";
  }
  ...
  if (!isHome) {
    ...
    if (
      subtitle?.dataset.circleDefaultText &&
      subtitle.textContent !== subtitle.dataset.circleDefaultText
    ) {
      subtitle.textContent = subtitle.dataset.circleDefaultText;
    }
    return;
