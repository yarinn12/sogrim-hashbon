...
function renderInsightPanel(state, event) {
  ...
  panel.innerHTML = `
    ...
    <div class="event-insight-metrics" aria-label="מצב האירוע">
      ...
      <div><span>סך האירוע</span><strong class="amount">${formatCurrency(totalExpenses, currency)}</strong></div>
      <div><span>פתוח להעברה</span><strong class="amount">${formatCurrency(sumPending(pendingTransfers), currency)}</strong></div>
    </div>
  `;
  return panel;
}
...
function renderSettlementHero(state, event) {
  ...
  hero.innerHTML = `
    <div class="settlement-hero-main">
      ...
      <strong class="settlement-hero-amount amount">${formatCurrency(pendingTotal, currency)}</strong>
    </div>
