...
test("internal screens keep back controls while dialogs keep one close action", async () => {
  ...
  assert.doesNotMatch(
    studio,
    /\.profile-setup-screen \.product-app-nav \{[\s\S]*?display: none !important/
  );
  assert.match(
    studio,
    /\.profile-first-run-screen \.product-app-nav \{[\s\S]*?display: none !important/
  );
});

test("home lives in app navigation and focused task controls", async () => {
  const [layer, workspace, app] = await Promise.all([
    readFile("src/publicHomeButtonLayer.mjs", "utf8"),
  ...
  assert.match(
    workspace,
    /\.profile-first-run-screen \.product-app-nav,[\s\S]*?\.profile-first-run-screen[\s\S]*?\.app-back-button:disabled \{[\s\S]*?display: none !important/
  );
  assert.match(app, /profile-edit-screen" : "profile-first-run-screen"/);
