...
test("QR uses a compact cloud invite while copied links keep their full fallback", () => {
  const fullInvite =
    "https://sogrim-hashbon.vercel.app/?event=event-1&space=space-1&key=abcdefghijklmnopqrstuvwxyzABCDEF&invite=large-snapshot";
  const compactInvite = compactQrInviteUrl(fullInvite);

  assert.equal(
    compactInvite,
    "https://sogrim-hashbon.vercel.app/i/event-1/space-1/abcdefghijklmnopqrstuvwxyzABCDEF"
  );
