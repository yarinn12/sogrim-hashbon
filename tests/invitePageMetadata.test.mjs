...
test("the server serves revocable token invite paths without exposing the token", async () => {
  const server = createServer(createAppHandler({ root: process.cwd(), port: 0 }));
  await new Promise((resolve, reject) => {
    server.once("error", reject);
    server.listen(0, "127.0.0.1", resolve);
  ...
  try {
    const { port } = server.address();
    const response = await fetch(
      `http://127.0.0.1:${port}/i/event-token/t/${token}`
    );
    const document = await response.text();

    assert.equal(response.status, 200);
    assert.match(document, /<base href="\/"/);
    assert.match(document, /sogrim-share-logo\.png/);
    assert.doesNotMatch(document, new RegExp(token));
  } finally {
    await new Promise((resolve, reject) => {
      server.close((error) => (error ? reject(error) : resolve()));
    });
