...
test("web app manifest declares an installable mobile app", async () => {
  ...
  assert.equal(manifest.start_url, "./?pwa_release=413");
  assert.equal(manifest.theme_color, "#0b3b38");
  assert.deepEqual(manifest.categories, ["finance", "productivity", "utilities"]);
  assert.equal(manifest.shortcuts[0].url, "./?pwa_release=413&action=new-event");
  assert.ok(
    manifest.icons.some(
...
test("index links the manifest and mobile app metadata", async () => {
  ...
  assert.match(
    html,
    /rel="manifest" href="\.\/manifest\.webmanifest\?pwa_release=413"/
  );
  assert.match(html, /data-pwa-release="413"/);
  ...
  assert.match(html, /href="\.\/styles\.css\?pwa_release=413"/);
  assert.match(html, /src="\.\/src\/pwaBootstrap\.mjs\?pwa_release=413"/);
  assert.match(html, /publicInstallAppLayer\.mjs\?pwa_release=413/);
});
...
test("the recovery page installs the current worker before reopening the app", async () => {
  ...
  assert.match(page, /src="\.\/src\/pwaRecovery\.mjs\?pwa_release=413"/);
  assert.match(recovery, /const PWA_RELEASE = "413"/);
