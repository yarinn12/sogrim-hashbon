...
async function collectBrowserModules(entryPaths) {
  ...
  while (pending.length) {
    ...
    for (const match of imports) {
      const resolved = `/${path.normalize(path.join(path.dirname(modulePath), match[1]))}`;
...
test("server serves install icons with the correct image type", async () => {
  ...
  assert.ok(config.builds[0].config.includeFiles.includes("icon-192.png"));
  assert.ok(config.builds[0].config.includeFiles.includes("icon-maskable-512.png"));
  assert.ok(config.builds[0].config.includeFiles.includes("apple-touch-icon.png"));
});
...
test("app registers the service worker when the browser supports it", async () => {
  ...
  assert.match(app, /if \(!hadController \|\| reloadingForUpdate\) return/);
  assert.match(app, /reloadingForUpdate = true/);
