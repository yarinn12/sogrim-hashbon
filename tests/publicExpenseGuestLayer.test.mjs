...
test("public expense guest layer loads after the command icon layer", async () => {
  ...
  assert.match(
    index,
    /publicCommandIconLayer\.mjs\?pwa_release=406"><\/script>\s+<script type="module" src="\.\/src\/publicExpenseGuestLayer\.mjs\?pwa_release=406"><\/script>/
  );
});
...
test("public inline payer layer loads after expense guest controls", async () => {
  ...
  assert.match(
    index,
    /publicExpenseGuestLayer\.mjs\?pwa_release=406"><\/script>\s+<script type="module" src="\.\/src\/publicInlinePayerLayer\.mjs\?pwa_release=406"><\/script>/
  );
