...
test("quick expense batches wait for a durable result and emit one bounded activity", () => {
  ...
  assert.match(quickSave, /const saveResult = await persistState\(\)/);
...
test("legacy permission and participant alias actions await cloud persistence", () => {
  ...
  assert.match(alias, /const result = await persistState\(\)/);
...
test("event participant mutations await persistence and recover after failure", () => {
  ...
  assert.match(addGuest, /const saveRequest = persistState\(\)/);
  ...
  assert.match(separateDuplicates, /const result = await persistState\(\)/);
  assert.match(renameOffline, /const result = await persistState\(\)/);
