...
test("Hebrew font loading does not block the app from rendering", async () => {
  ...
  assert.match(html, /src="\.\/src\/publicFontLoader\.mjs\?pwa_release=499"/);
...
test("event rows render participants, a uniform text status, and a quiet options chevron", async () => {
  ...
  assert.doesNotMatch(
    app.slice(app.indexOf("function renderEventRow(event, pinned = false)"), app.indexOf("function ensureNewEventDraft")),
