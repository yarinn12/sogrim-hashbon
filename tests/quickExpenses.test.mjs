...
test("restaurant quick entry uses a useful default name when the description is blank", () => {
  const result = buildQuickItemExpenses({
  ...
  assert.equal(result.error, "");
  assert.equal(result.expenses[0].name, "מנה 1");
  assert.equal(result.expenses[0].total, 4500);
});
...
test("four-person restaurant split stays exact through the final settlement", () => {
  let id = 0;
  const participantIds = ["yarin", "dani", "maor", "avi"];
  const items = [
    { name: "Pasta", amount: "58", sharedBy: "avi" },
    { name: "Shared salad", amount: "42.50", sharedBy: QUICK_ITEM_ALL_PARTICIPANTS },
    { name: "Drinks", amount: "30", sharedBy: "dani" }
  ];
  const summary = summarizeQuickItemShares(items, participantIds);
  const built = buildQuickItemExpenses({
    items,
    payerParticipantId: "yarin",
    participantIds,
  ...
  assert.equal(summary.error, "");
  assert.equal(summary.billTotal, 13050);
  ...
  assert.deepEqual(
    settlement.transfers.map(({ fromParticipantId, toParticipantId, amount }) => ({
      fromParticipantId,
      toParticipantId,
      amount
    })),
    [
      { fromParticipantId: "avi", toParticipantId: "yarin", amount: 6862 },
      { fromParticipantId: "dani", toParticipantId: "yarin", amount: 4063 },
      { fromParticipantId: "maor", toParticipantId: "yarin", amount: 1062 }
    ]
  );
});
