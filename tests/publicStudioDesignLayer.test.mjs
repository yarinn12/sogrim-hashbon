...
test("studio design layer defines the production visual system", async () => {
  ...
  assert.match(layer, /EVENT_TYPE_ICONS\[option\.dataset\.eventType\]/);
  assert.match(layer, /EVENT_TYPE_ICONS\.standard/);
  assert.doesNotMatch(layer, /EVENT_TYPE_ICONS\[index\]/);
