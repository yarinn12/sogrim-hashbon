...
test("event creation and settings expose an event-level currency", async () => {
  ...
  assert.match(app, /const currencyLocked = event\.expenses\.length > 0/);
  assert.match(app, /!canManage \|\| currencyLocked \? "disabled"/);
  assert.match(app, /המטבע ננעל אחרי שנוספה הוצאה/);
});
