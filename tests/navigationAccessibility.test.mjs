...
test("every screen transition moves focus to the new screen heading", async () => {
  ...
  assert.match(index, /<main id="app" class="app app-boot font-hebrew" tabindex="-1" aria-busy="true">/);
  ...
  assert.match(
    app,
    /function canNavigateBackWithinApp\(\) \{[\s\S]*?appHistoryDepth > 0[\s\S]*?screen\.name !== "home"[\s\S]*?eventDialog/
  );
...
test("mobile keyboard actions advance fields and submit safe inline actions", async () => {
  ...
  assert.match(
    app,
    /next instanceof HTMLSelectElement && next\.hidden[\s\S]*?app-choice-trigger[\s\S]*?next = choiceTrigger/
  );
...
test("focused tasks keep back and home in quick controls", async () => {
  ...
  assert.match(app, /new CustomEvent\("settle-friends:screen-rendered"\)/);
  assert.match(layer, /product-route-controls/);
  assert.match(layer, /document\.addEventListener\(SCREEN_RENDERED_EVENT, syncHomeButton\)/);
...
test("event navigation exposes its current section and explicit button types", async () => {
  const app = await readFile("src/app.mjs", "utf8");
  const start = app.indexOf("function renderEventWorkspaceNav(event, activeView");
  ...
  assert.match(navigation, /event-workspace-summary\$\{summaryIsActive/);
...
test("task dialogs connect their title and explanatory copy for screen readers", async () => {
  const app = await readFile("src/app.mjs", "utf8");

  assert.match(
    app,
    /aria-labelledby="event-modal-title"[\s\S]*?aria-describedby="event-modal-description"/
  );
  assert.match(app, /id="event-modal-description"/);
  assert.match(
    app,
    /aria-labelledby="expense-modal-title" aria-describedby="expense-modal-description"/
  );
  assert.match(app, /id="expense-modal-description"/);
  assert.match(
    app,
    /aria-labelledby="quick-expense-title"[\s\S]*?aria-describedby="quick-expense-description"/
  );
  assert.match(app, /id="quick-expense-description"/);
});
