...
test("circle design layer defines the final product identity", async () => {
  ...
  assert.match(layer, /brand-mark-v3\.png/);
...
test("home stays selection focused while event actions remain available", async () => {
...
test("home uses one compact list for every event", async () => {
  ...
  assert.match(layer, /font-family: "IBM Plex Sans Hebrew", "Noto Sans Hebrew"/);
});
