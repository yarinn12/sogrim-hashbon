...
-- Keep shared notes immediately visible in every active member's personal
-- workspace. This runs in the canonical snapshot transaction, so the source
-- and all device-facing indexes either advance together or all roll back.
...
-- Redeem an invite, publish canonical membership and index the recipient's
-- workspace in one database transaction. A failed or suspended HTTP request
-- can no longer leave a durable half-join between those states.
create or replace function public.redeem_event_invite_membership(
  p_invite_id uuid,
  p_token_hash text,
  p_user_id uuid
...
-- Keep personal note projections authoritative even after a stale client save.
