-- An active server membership must stay represented by the canonical account
-- participant inside the matching personal event. This extends the event-index
-- guard so a stale device cannot retain the event while silently removing the
-- signed-in member from its participant list.
