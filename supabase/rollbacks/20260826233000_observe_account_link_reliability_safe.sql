...
begin
  if exists (
    ...
    where event_name in ('operation_deferred', 'operation_failure')
      and detail ~ '^account_link(:|$)'
  ) then
    raise exception 'Rollback refused: account-link reliability metrics already exist';
  end if;
end;
$$;
