...
export async function purchasePremium(config, fetchImpl = fetch) {
  assertAvailable(config);
  const purchase = await billingPlugin().purchaseSubscription({
    ...productOptions(config),
    accountId: config.storage.account.userId
  });
  return verifyNativePurchase(config, purchase, fetchImpl);
}

export async function restorePremium(config, fetchImpl = fetch) {
  assertAvailable(config);
  ...
  for (const purchase of purchases) {
    try {
      const candidate = await verifyNativePurchase(config, purchase, fetchImpl);
...
export async function observePremiumPurchases(
  ...
  return plugin.addListener("purchaseUpdated", async (purchase) => {
    ...
    try {
      const result = await verifyNativePurchase(config, purchase, fetchImpl);
...
async function verifyNativePurchase(config, purchase, fetchImpl) {
