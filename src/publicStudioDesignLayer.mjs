...
const CSS = `
  ...
  html.product-studio-v3 .profile-setup-screen .product-app-nav {
    display: none !important;
  }

  html.product-studio-v3 .profile-setup-screen .product-app-identity {
    grid-template-columns: 1fr !important;
  }

  html.product-studio-v3 .profile-setup-screen .product-brand-lockup {
    justify-self: start !important;
  }
...
function enhanceInterface() {
  ...
  document.querySelectorAll(".event-type-option").forEach((option, index) => {
    if (option.querySelector(".studio-event-type-icon")) return;

    const icon = document.createElement("span");
    icon.className = "studio-event-type-icon";
    icon.setAttribute("aria-hidden", "true");
    icon.innerHTML = EVENT_TYPE_ICONS[index] ?? EVENT_TYPE_ICONS[0];
    option.prepend(icon);
  });
}
