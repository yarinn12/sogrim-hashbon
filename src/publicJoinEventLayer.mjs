...
function enhanceHomeActions(screen) {
  ...
  const hasNativeJoinAction = Boolean(screen.querySelector('[data-action="join-event-screen"]'));

  if (heroActions && !hasNativeJoinAction) {
...
async function joinExistingEventFromPublicPanel() {
  ...
  const inviteCredentials = {
    id: parseInviteSpaceId(link),
    key: parseInviteSpaceKey(link)
  };
