...
const INSTALL_HISTORY_KEY = "settleFriendsInstallDialog";
...
let installHistoryActive = false;
let installHistoryClosing = false;
...
window.addEventListener("popstate", handleInstallHistoryBack, true);
window.addEventListener(NATIVE_BACK_EVENT, handleInstallNativeBack, true);
      ...
      <div class="install-app-backdrop">
        <section class="install-app-dialog" role="dialog" aria-modal="true" aria-labelledby="install-app-title" tabindex="-1">
          <header>
            ...
            <button class="icon-button modal-close-button" data-public-close-install type="button" aria-label="סגור" title="סגור"><span class="modal-control-icon" aria-hidden="true">${iconSvg("x")}</span></button>
          </header>
          ${instructions}
...
function closeInstallDialog({ fromHistory = false } = {}) {
  const backdrop = document.querySelector(".install-app-backdrop");
  if (!backdrop) return;
  if (!fromHistory && installHistoryActive && window.history?.back) {
    if (installHistoryClosing) return;
    installHistoryClosing = true;
    window.history.back();
    return;
  }

  installHistoryActive = false;
  installHistoryClosing = false;
  backdrop.remove();
...
function pushInstallHistoryState() {
  if (!window.history?.pushState) return;
  try {
    window.history.pushState(
      {
        ...(window.history.state ?? {}),
        [INSTALL_HISTORY_KEY]: true
      },
      "",
      window.location.href
    );
    installHistoryActive = true;
  } catch {
    installHistoryActive = false;
  }
}

function handleInstallHistoryBack(event) {
  if (!installHistoryActive) return;
  event.stopImmediatePropagation();
  closeInstallDialog({ fromHistory: true });
}

function handleInstallNativeBack(event) {
  if (!document.querySelector(".install-app-backdrop")) return;
  event.preventDefault();
  event.stopImmediatePropagation();
  closeInstallDialog();
}
