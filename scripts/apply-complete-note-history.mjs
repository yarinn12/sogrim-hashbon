...
const migration = await readFile(
  "supabase/migrations/20260903223000_preserve_complete_note_history.sql", "utf8"
);
const verification = await readFile(
  "supabase/verification/verify_20260903223000_preserve_complete_note_history.sql", "utf8"
);
...
try {
  if (dryRun) {
    ...
    console.log("Complete note history verified with rollback, including authorization checks.");
  } else {
    if (apply) await sql.unsafe(migration);
    await sql.unsafe(verification);
    console.log(apply
      ? "Complete note history applied and verified."
      : "Complete note history is active.");
  }
} finally {
