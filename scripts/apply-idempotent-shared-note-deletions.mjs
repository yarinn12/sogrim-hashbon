...
const migration = await readFile(
  "supabase/migrations/20260903213000_idempotent_shared_note_deletions.sql", "utf8"
);
const verification = await readFile(
  "supabase/verification/verify_20260903213000_idempotent_shared_note_deletions.sql", "utf8"
);
...
try {
  if (dryRun) {
    ...
    console.log("Idempotent note deletion verified with rollback, including authorization checks.");
  } else {
    if (apply) await sql.unsafe(migration);
    await sql.unsafe(verification);
    console.log(apply
      ? "Idempotent note deletion applied and verified."
      : "Idempotent note deletion is active.");
  }
} finally {
