...
const migrationName = "20260829154500_atomic_shared_event_member_indexes";
const [migration, verification] = await Promise.all([
  readFile(resolve(root, `supabase/migrations/${migrationName}.sql`), "utf8"),
  readFile(
    resolve(
      root,
      "supabase/verification/verify_20260829154500_atomic_shared_event_member_indexes.sql"
    ),
    "utf8"
  )
]);
